...
export const HeaderLargeTitleStore = ({
  ...
  onChange = () => {},
  style = { marginBottom: 18, fontStyle: "Italic" },
}) => {
...
export const HeaderLargeTitleBadge = ({ onPress, haveNotification }) => {
  const { colors } = useTheme();
  return (
    <TouchableOpacity
      ...
      onPress={onPress}
    >
      <Image source={Images.notification} style={{ width: 25, height: 25 }} />
      {/* <Icon name={"bell"} size={22} /> */}
      {haveNotification ? (
        <View
          style={[styles.badge, { backgroundColor: colors.primaryBlueColor }]}
        />
      ) : null}
    </TouchableOpacity>
  );
